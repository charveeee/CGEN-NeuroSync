...
# Process metrics stream (Crucial for the dashboard chart simulator!)
@app.post("/analyze")
async def analyze_stream(payload: EEGPayload):
    features = processor.clean_and_extract_features(np.array(payload.raw_signals))
...
# 1. Main Home/Landing Route
@app.get("/")
async def read_index():
    return FileResponse('static/index.html')

# 2. Console/Dashboard Route (Allows streaming connection to talk to dashboard!)
@app.get("/console.html")
async def read_console():
    return FileResponse('static/console.html')

# 3. Glossary/Docs Route
@app.get("/docs.html")
async def read_docs():
    return FileResponse('static/docs.html')
